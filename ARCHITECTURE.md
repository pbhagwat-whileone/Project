...
* **Frontend:** Next.js (App Router) — Provides the React-based presentation layer, dashboard, and settings interfaces.
...
* **AI Layer:** 
  * **Cerebras:** The primary ultra-fast LLM engine for generation, extraction, and analysis.
  * **Gemini:** Used for generating vector embeddings and fallback generation.
  * **Claude:** Reserved for optional advanced reasoning tasks.
* **External Services:**
  ...
  * **Google Drive / Google OAuth:** Ingests internal WhileOne documentation (case studies, project briefs) directly from user drives.

**Responsibilities:** The frontend handles user intent; the backend coordinates data pipelines; Supabase stores state and vectors; Tavily/Apollo fetch external reality; and Cerebras/Gemini synthesize the data into structured outputs.
...
3. **Intelligence Layer (Cerebras / Claude):** 
...
The Knowledge Base transforms WhileOne's unstructured internal documents into searchable semantic vectors used for proof points.
...
* **LLM Structuring:** The raw search results are fed into Cerebras, which is instructed to extract JSON arrays of specific entities.
* **Data Extracted:**
...
* **Analysis:** All historical messages are fed to Cerebras.
...
* **Relevance Scoring:** Cerebras evaluates the findings and outputs a `CompanyContextRelevance` score. 
...
* **Cache Lifecycle:** Stored with an `expires_at` column. If expired, a background job re-fetches the intelligence.
...
* **Email Usage:** The matched chunk text is provided to the email generator. The generator is strictly instructed to extract business outcomes (e.g., "Reduced validation effort by 40%") rather than technical jargon to prove capability.
...
### Inputs
The generator function receives: `targetCompany`, `contact`, matched `projects`, `relationshipIntelligence`, `companyContextRelevance`, `companyContext`, message history metrics, and connection metadata.

### Context Hierarchy
The prompt strictly enforces this hierarchy:
1. **Outreach Angle Selection** (Priority: Hiring Signal > Initiative > Activity > Tech Tag > Role)
2. **Proof** (Matching Projects > Technical Assets > Generic Experience)
3. **Personalization** (Relationship/Conversation history used *only* for tone/CTA).

### Prompt Assembly
The pipeline merges the dynamic Context Hierarchy with the Markdown file from `skills/[relationship].md` (which sets familiarity and tone) and the system-wide `WHILEONE_MESSAGING_RULES`.
...
Output is stored as `{ "subject": "...", "body": "..." }`.
...
* **Extraction:** Cerebras parses unstructured search results into structured event schemas (Name, Date, URL, Relevance).
* **Caching:** Cached in memory or dedicated database tables to drive the Discover UI.

---

## 17. AI Architecture

The platform utilizes a multi-model factory pattern (`src/ai/providers/factory.ts`).

* **Cerebras (Llama 3 / Fast Models):** 
  * *Purpose:* Workhorse for all high-volume, synchronous tasks. Used for email generation, JSON extraction (enrichment), intelligence classification, and relevance scoring. Chosen for extreme speed.
* **Gemini:** 
  * *Purpose:* Required specifically for embedding generation (`text-embedding-004`). Used as a fallback LLM provider if Cerebras fails or hits rate limits.
* **Claude (Anthropic):** 
  * *Purpose:* Optional provider configured for tasks requiring advanced temporal reasoning or highly nuanced context synthesis, though primarily kept as a backup to Cerebras.
...
* `connections`: Central entity for people. Contains network data and metadata.
* `connection_profiles`: Enriched Tavily data (tags, locations). 1-to-1 with connections.
* `linkedin_messages`: Raw message history. Deduplicated by hash.
* `connection_relationship_metrics`: AI-analyzed intelligence derived from `linkedin_messages`.
...
**Common Constraints:** 
Almost all tables require a `user_id` foreign key mapped to `profiles(id)` and enforce Row Level Security.
...
* **Profile Cache (`connection_profiles`):** Soft TTL. Refreshed manually via UI or via long-running chron jobs.
* **Company Context (`company_context_cache`):** Hard TTL via `expires_at` column. Ensures "Why Now" signals (like hiring or news) remain fresh but aren't queried on every email generation.
* **Score Cache (`company_score_cache`):** Recalculated asynchronously when new messages or connections are imported.
...
Next.js API routes (`src/app/api/`) serve as the network boundary.
...
* **`/api/emails`:** The generation controller. Gathers data from 5+ database tables, constructs the prompt, invokes the AI Factory, and returns the JSON payload.
* **`/api/company` & `/api/prospects`:** Feeds the dashboard with scored targets and orchestrates Apollo discovery.
* **`/api/search`:** Specialized route for executing the `match_knowledge_chunks` RPC.
...
`Login ↓ Link Google OAuth ↓ Ingest Drive Folders (Knowledge Base) ↓ Upload LinkedIn Connections CSV ↓ Upload LinkedIn Messages CSV ↓ Background Processing (Enrichment/Scoring) ↓ Ready to Generate Outreach`

### B. Company Discovery Flow
`View Dashboard (Company Recommendation) ↓ Select High-Scoring Company ↓ View Company Analysis (Tavily Cache) ↓ Trigger Stakeholder Discovery (Apollo) ↓ Filter out existing connections ↓ Select Prospect ↓ Generate Email`

### C. Relationship-Based Outreach Flow
`Select Existing Connection ↓ System fetches Conversation Analysis ↓ System applies Relationship Intelligence (e.g., dormant-relationship) ↓ System executes Project Matching via pgvector ↓ Generator strictly applies context hierarchy ↓ LLM outputs 7-Block Email JSON`

---

## 22. Deployment Architecture

* **Application:** Deployed as a Next.js Docker container (or on Vercel), utilizing serverless edge functions for API routes where possible.
* **Database:** Supabase acts as a standalone managed PostgreSQL cluster.
* **Dependencies:** Requires outbound network access to Cerebras, Gemini, Tavily, Apollo, and Google APIs.
* **Environment Variables:** Configuration relies heavily on `.env` injected at runtime containing API keys for all External Integration Layer services and Supabase connection strings.
